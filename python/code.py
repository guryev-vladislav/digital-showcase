...
def create_frequency_table(results):
    """Creates a frequency table of observed values."""
    counter = collections.Counter(results)
    sorted_items = sorted(counter.items())
    yi = [item[0] for item in sorted_items]
    ni = [item[1] for item in sorted_items]
    return {"yi": yi, "ni": ni, "ni/n": [i / len(results) for i in ni]}
...
def create_characteristics_table_png(characteristics):
    """Creates a PNG image of the characteristics table."""
    ...
    df = df.rename(columns={
        ...
        "Dη": "Dη",
        "S^2": "S^2",
        "abs_diff_variance": "|Dη - S^2|",
        "Me": "Me",
        "Rb": "Rb"
    })

    fig, ax = plt.subplots(figsize=(10, 1))  # Adjust figsize as needed
...
def print_frequency_table(frequency_table):
    """Prints the frequency table nicely."""
    print("Значение (y)\tЧастота (n_y)")
    for y, count in sorted(frequency_table.items()):
        print(f"{y}\t{count}")


def plot_cdf(results, lambd, t):
    """Plots the empirical and theoretical CDFs."""
    ecdf = np.sort(results)
    cdf_poisson = [stats.poisson.cdf(k, lambd * t) for k in range(min(results), max(results) + 1)]
    ecdf_vals = np.arange(1, len(ecdf) + 1) / len(ecdf)
    plt.figure(figsize=(10, 6))
    plt.plot(ecdf, ecdf_vals, marker='.', linestyle='none', label='Эмпирическая ФРВ')
    plt.plot(range(min(results), max(results) + 1), cdf_poisson, label='Теоретическая ФРВ')
    plt.xlabel("Значение случайной величины (η)")
    plt.ylabel("Функция распределения")
    plt.title("Эмпирическая и теоретическая функции распределения Пуассона")
    plt.legend()
    plt.show()
...
def main():
    ...
    table_png_buf = create_characteristics_table_png(characteristics)
    
    # Show the PNG table as a plot
    img = plt.imread(table_png_buf, format='png')
    plt.imshow(img)
    plt.axis('off')
    plt.show()
